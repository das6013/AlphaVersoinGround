...
public class Generator : MonoBehaviour {
    ...
    List<Branch> fruitBranch = new List<Branch>();
    List<Branch> pBranch = new List<Branch>();
    ...
    [SerializeField] private int _trunkHeight;

    int _lastAttracts;
    //[SerializeField]GameObject pTree;
	void Awake () {
		// initilization 
	}
	...
	void GenerateAttractors (int n, float r) {
		for (int i = 0; i < n; i++) {
			...
			secondPt+= transform.position + Vector3.up*7;
            Vector3 pt = secondPt;
            //secondPt.y += 7;

            _attractors.Add(secondPt);
		}
	}
	...
	void Start () {
        ...
        //_startPosition.y=_startPosition.y - _trunkHeight;
        ...
        _lastAttracts = _nbAttractors;

        
    }
    int maxDist = 0;
    public void GrowUpOnAmount(int _amount)//range beetwen 1-120
    {
        if (_nbAttractors > 0)
        {
            ...
            ToMesh();
            foreach (Branch ex in _extremities)
            {
    ...
    public void LogicGrowing()
    {
        ...
        if (_attractors.Count > 0)
        {
            ...
            foreach (Branch b in _branches)
            {
                b._attractors = new List<Vector3>();
            }
            ...
            else
            {
                float _minD = 99999f;
                Branch closest = null;
                foreach (Vector3 _lastAttract in _attractors)
                {
                    
                    foreach(Branch br in _branches)
                    {
                        if(_minD> Vector3.Distance(br._end,_lastAttract))
                        {
                            _minD = Vector3.Distance(br._end, _lastAttract);
                            closest = br;
                        }
                    }
                ...
                if (closest != null)
                {
                    ...
                    Vector3 start = closest._end;
                    // we add randomness to the direction
                    Vector3 dir = closest._direction + RandomGrowthVector();
                    // we add the direction multiplied by the branch length to get the end point
                    Vector3 end = closest._end + dir * _branchLength;
                    // a new branch can be created with the same direction as its parent
                    Branch nb = new Branch(start, end, dir, closest);
                    nb._distanceFromRoot = closest._distanceFromRoot + 1;
                    closest._children.Add(nb);
                    _branches.Add(nb);
                    _extremities.Add(nb);
                }
                ...
                /*for (int i = 0; i < _extremities.Count; i++)
                {
                    ...
                    nb._distanceFromRoot = e._distanceFromRoot + 1;
    ...
    void setFruitSpawn()
    {   
        fruitBranch = new List<Branch>(_extremities);
        fruitPoints = new List<Vector3>();
        //int lastCount = 0;
        
     	foreach(Branch branch in _extremities)
    	{
            Branch bParent = branch._parent;
		while(bParent._children.Count < 2)
		{
                fruitBranch.Add(bParent);

            bParent = bParent._parent;
		}
    	}
	
	foreach (Branch b in fruitBranch) 	
		fruitPoints.Add(b._end);
    	
	    
	    
        //foreach(Branch b in fruitBranch) if (b._parent != null) pBranch.Add(b._parent); 
        /*for(int i = 0;i<15;i++)
        {
       
            if (fruitBranch.Count - lastCount > 0)
            {

                for (; lastCount < fruitBranch.Count - 1; lastCount++)
                {
                    if (fruitBranch[lastCount]._parent != null)
                    {

                        if (fruitBranch[lastCount]._parent._children.Count<2)
				pBranch.Add(fruitBranch[lastCount]._parent);
                    }
                    else Debug.Log("Null parent");
                }
                fruitBranch.AddRange(pBranch);
                pBranch.Clear();
                //lastCount = fruitBranch.Count;
            }
        }*/
	...
	void OnDrawGizmos () {
		...
		*/
        ...
        foreach(Branch ex in fruitBranch)
        {
            Gizmos.color = Color.black;
            Gizmos.DrawSphere(ex._end, 0.05f);
            //Gizmos.color = Color.blue;
            //Gizmos.DrawSphere(ex._start, 0.05f);
        }
        foreach (Branch pb in pBranch)
        {
            Gizmos.color = Color.black;
            Gizmos.DrawSphere(pb._end, 0.05f);
            //Gizmos.color = Color.blue;
            //Gizmos.DrawSphere(ex._start, 0.05f);
        }
